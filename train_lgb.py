...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train LightGBM on handcrafted features and save test probabilities."
    )
    parser.add_argument(
        "--data-dir",
        default="data",
    ...
    parser.add_argument(
        ...
        help="Absolute or relative path to save the LightGBM test probabilities (.npy)",
    )
    parser.add_argument(
        "--seed", type=int, default=42, help="Random seed for LightGBM training"
    )
    parser.add_argument(
        "--n-estimators", type=int, default=500, help="Number of trees"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Add advanced cross-axis interaction channels that are not part of the original LGBM baseline.",
    )
...
def parse_manual_class_weights(spec: str, num_classes: int) -> dict[int, float]:
    if not spec.strip():
        raise ValueError(
    ...
    return {class_id: float(values[class_id]) for class_id in range(num_classes)}
...
def resolve_class_weight(
    ...
    base_weights = compute_class_weights(train_labels)
    weight_dict = {class_id: float(base_weights[class_id]) for class_id in range(num_classes)}
    for label, multiplier in boosts.items():
        if label < 0 or label >= num_classes:
            raise ValueError(f"Label {label} is out of range for {num_classes} classes.")
        weight_dict[label] *= multiplier
    return weight_dict
...
def main():
    ...
    if X_test is not None:
        lgb_test_probs = np.zeros((len(X_test), num_classes))
    ...
    print(f"Training LightGBM with seed {args.seed}...")
    for fold_id, (tr_idx, val_idx) in enumerate(folds):
        ...
        if isinstance(class_weight, dict):
            print(f"Fold {fold_id} class_weight: {class_weight}")
        ...
        # 計算 validation 預測結果與 macro F1-score
        ...
        # 累加各 fold 的測試集預測機率
        if X_test is not None and lgb_test_probs is not None:
            lgb_test_probs += clf.predict_proba(X_test) / num_folds

    # 計算整體的 OOF macro F1-score
    ...
    # 確保輸出路徑的資料夾存在並存檔
    if lgb_test_probs is not None:
        output_path = Path(args.output_probs)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        np.save(output_path, lgb_test_probs)
        print(f"Successfully saved LightGBM test probabilities to {output_path}")
